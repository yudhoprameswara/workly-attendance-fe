...
const ListView: React.FC<ListViewProps> = ({ data = [] }) => {
  ...
  if (lat >= -6.35 && lat <= -6.25 && lng >= 106.60 && lng <= 106.75) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center py-20 opacity-40">
        ...
        <p className="font-black uppercase tracking-widest text-[10px] text-neutral-500">
          Belum ada riwayat
        </p>
      </div>
    );
        ...
        return (
          ...
          >
            <div className="flex items-center gap-5">
              <div
                className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center shrink-0 border-2 transition-colors duration-300 ${isLate ? "bg-red-50 border-red-100 text-red-600" : "bg-blue-50 border-blue-100 text-blue-600"}`}
              >
              ...
              <div className="space-y-1.5 md:text-right">
                <div className="flex items-center md:justify-end gap-1.5 text-neutral-400">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">
                    {item.device_info
                      ? item.device_info.split(" ")[0]
                      : "Unknown"}
                  </span>
                </div>
                <div className="flex items-center md:justify-end gap-1.5 text-neutral-900">
                  <MapPin size={12} />
                  <span className="text-[12px] text-neutral-900  " >{getSimpleLocation(item.location_in)}</span>
                </div>
              </div>

              <div
                className={`px-4 py-2 rounded-2xl flex items-center gap-2 border-2 transition-all duration-300 ${isLate ? "bg-red-50 border-red-100 text-red-600" : "bg-green-50 border-green-100 text-green-600"}`}
              >
